...
{
    public static Task<Results<Ok<GetPaymentResponse>, NotFound, ValidationProblem, ProblemHttpResult>>
        HandleAsync(
            [FromRoute] Guid referenceId,
            ILogger<GetPaymentByReferenceHandler> logger
        )
    {
        return Task.FromResult<Results<Ok<GetPaymentResponse>, NotFound, ValidationProblem, ProblemHttpResult>>(
            TypedResults.Ok(
